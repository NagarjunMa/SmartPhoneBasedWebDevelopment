...
class ArtistService {
    ...
    private init() {
        ...
        let path = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!.appending("/AssignmentEight.sqlite3")
    ...
    func albumDisplay() -> String {
        ...
        if sqlite3_prepare_v2(db, queryStatementString, -1, &queryStatement, nil) == SQLITE_OK {
            while sqlite3_step(queryStatement) == SQLITE_ROW {
                ...
                result += "Album ID: \(id), Artist ID: \(artistId), Title: \(title), Release Date: \(releaseDate)\n"
            }
        } else {
            print("SELECT statement could not be prepared.")
        }
    ...
    func genreDisplay() -> String {
        ...
        if sqlite3_prepare_v2(db, displayStatementString,-1, &displayStatement,nil) == SQLITE_OK {
            while sqlite3_step(displayStatement) == SQLITE_ROW {
                let id = sqlite3_column_int(displayStatement, 0)
                guard let queryResultCol2 = sqlite3_column_text(displayStatement, 1) else {
                    print("Query result is nil.")
                    continue
                }
                let title = String(cString: queryResultCol2)
                result += "Genre ID: \(id), Genre Title: \(title), \n"
            }
        } else {
            print("SELECT statement could not be prepared")
        }
...
//        return songs.contains { $0.id == songId}
    ...
    func songDisplay() -> String {
        ...
        if sqlite3_prepare_v2(db, displayStatementString, -1, &displayStatement, nil) == SQLITE_OK {
            while sqlite3_step(displayStatement) == SQLITE_ROW {
                let id = sqlite3_column_int(displayStatement, 0)
                let artistId = sqlite3_column_int(displayStatement, 1)
                let albumId = sqlite3_column_int(displayStatement, 2)
                let genreId = sqlite3_column_int(displayStatement, 3)
                ...
                let title = String(cString: queryResultCol2)
                let duration = String(cString: queryResultCol3)
                let favorite = sqlite3_column_int(displayStatement,6)
                let isFavorite = favorite != 0
                result += "Song ID: \(id), Artist ID: \(artistId), Album ID: \(albumId),Genre ID: \(genreId),Song Title: \(title),Song Duration : \(duration), Favorite: \(isFavorite)\n"
            }
        } else {
            print("SELECT statement could not be prepared.")
        }
